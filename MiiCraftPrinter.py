...
'''
 ...
 V1.0    2021-Mar-24th [Baramykin V.P.] 
            Change code for reading from .idx file thickness of layer.
            Add comand "z" for set up thickness layer. First send z 0.2second after sleep secondary send
                number of thickness of layer in microns: 5, 10, 15, 20, 25.. ect. 
            Change read Index file to run.gcode from CHITUBOX 64. Readed: layerHeight; totalLayer.
            Change menu ResinRedraw command from z to Z for twice longer time redrawing.
            Add one more commands for more redrawing before start. See coment: Add one more commands for more redrawing before start.
...
class CmdDialog:
    def __init__(self, parent, VariableName, Message):
        global ResinCuringTime, SerialTimeOut, PostCuringTime, FormBase, thickness_layer
    ...
    def ok(self, VariableName):
        global root, ResinCuringTime, SerialTimeOut, PostCuringTime, FormBase, thickness_layer
        ...
        if VariableName=="thickness_layer":                           
            thickness_layer = int(cmd[0])
            ConfigBtn.menu.entryconfig(5, label='Thickness of layer :   '+str(thickness_layer)+' microns')
            print "CmdDialog Thickness set =", thickness_layer
            return
...
def OpenFile():
    ...
    global dirname, extname, startname, endname
    global ini_start_num, ini_end_num, ini_base_start
    global start_num, end_num, total_num
    global FormBase
    global thickness_layer
    global ser
    
    ftypes = [('Image Index File', '.gcode')]
    newfilename = askopenfilename(filetypes=ftypes)
    if not newfilename: # no file has been selected
        return
    
  
    indexFile = open(newfilename, 'r')
    #indexFile = codecs.open(newfilename, 'r', encoding='utf-8')
    for line in indexFile:
        split_line = line.split(":")
       # print 'Line: ', split_line
        if split_line[0] == ';layerHeight':
            ini_thckn = float(split_line[1]) * 1000
            thickness_layer = int(ini_thckn)
        if split_line[0] == ';totalLayer':
            endname = int(split_line[1])            
    ...
    print "Thickness in microns: ", thickness_layer
    SettingThickness()
    CurrentDir.set("Directory :  "+ dirname)
    Prefix_Ext.set("No File Prefix:  "+"\t\t\tFile Ext:  "+extname)
    Start_End.set ("Start from:  "+startname+"\t\t\tEnd At :  "+endname)
    
    imageFile = dirname+"/"+startname+"."+extname
    ...
    PreviewName.set("Preview Images - "+startname+"."+extname)
...
"""# 20130128 Michael, Disable manual connecting function
COMList= enumerate_serial_ports() # Get the COM Port List
...
def RedrawResin_long():
    ...
    if cmdback[0]!="Z":
        #print "NVP no response to cmd i"
        StatusText.set("Printer no responses to cmd Z")
        root.update()
        return
...
def send_a_cmd(cmd, message):
    ...
    cmdback = NVPback
    print "Response to cmd "+cmdback
    compare = cmdback[0]
    if compare == 'L':    # Extract r OK from message: Layer thickness: 50 micron.r OK
        cmdback = cmdback[27:31]
...
def build_a_layer(layer_num):
    ...
    global dirpathname, extname, startname, endname
    ...
    imageFile = dirname+"/"+layername+"."+extname
    ...
    PreviewName.set("Preview Images - "+layername+"."+extname)
    root.update()
...
def ControlPrint():
    ...
    if state == 'R':
        ...
        while state == 'R':
            root.update()
    RedrawResin()
    RedrawResin()   # Add one more commands for more redrawing before start.
...
def SettingSpeed():
    ...
    if speed_print==0:  # old speed_print+thick_layer==0:
        MoveAlayer  = "c"   # old "d"
    if speed_print==1:  # old speed_print+thick_layer==1:
        MoveAlayer  = "e"   # old "f"
...
def SettingThickness():
    global speed_print, thick_layer, MoveAlayer, thickness_layer
    global ser
    #if thick_layer==0:
    PrintThickness.set(thickness_layer)
    ...
    print "send_a_cmd", NVPback
    time.sleep(0.2)
    ser.write(thickness_layer)
    print "set: ", thickness_layer
    ack =''
    while ack != 'OK':
        StatusText.set("Applyning thickness ...")
        #print "Applyning thickness ...", cmnd
        root.update()
        NVPback = ser.readline()
        if NVPback!='':
            cmdback = NVPback
            ...
            state = '0'
            ack = cmdback[0:2]
    StatusText.set("Thickness defined as " + str(thickness_layer) + " microns")
...
global ResinCuringTime, USBDisplayStartX, PostCuringTime, FormBase, thickness_layer
# defaults
...
StatusText.set("1 - Connect to Printer. 2 - Load a valid Idx file")
NVPConnectText.set("Printer NOT Connected")
Pause_Resume.set("Pause")
Initiate_updown.set("Initiate")
TimeRemainText.set(" ")
PrintSpeed.set("Normal")
PrintThickness.set("Apply")  # 100 micro
...
ResinBtn.menu.add_command(label='Redraw Resin', command=RedrawResin_long)
